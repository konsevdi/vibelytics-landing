...
| Fresh target date | On 2026-09-21, both production surfaces selected `2026-11-02`, exactly 42 calendar days ahead | Pass |
...
| Invalid calendar date | `date=2026-99-99` preserves the computed `2026-11-02` default and keeps email/share artifacts aligned | Pass in production |
| Leap-year boundary | `2028-02-29` restores exactly; invalid `2027-02-29` preserves the computed default | Pass in production |
...
- P2 resolved in production: query restoration validates Gregorian month lengths and leap years before replacing the computed default.
...
- Next: audit the unordered roadmap candidates and select exactly one bounded task before further implementation.
